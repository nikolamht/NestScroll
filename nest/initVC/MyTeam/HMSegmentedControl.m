...
{
    CGFloat OldDynamicHeaderTailer;
}
        ...
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
                ...
                for (NSNumber *width in self.segmentWidthsArray) {
                    if (idx == i)
                        break;
                    if (i+1 == self.sectionTitles.count) {
                        xOffset = xOffset + [width floatValue];
                    }else{
                        xOffset = xOffset + [width floatValue] + self.DynamicSpace;
                    }
                    i++;
                }
                
                CGFloat widthForIndex = [[self.segmentWidthsArray objectAtIndex:idx] floatValue];
                rect = CGRectMake(xOffset + self.DynamicHeaderTailer, y, widthForIndex, stringHeight);
                
                if (idx + 1 == self.sectionTitles.count) {
                    fullRect = CGRectMake(xOffset, 0, widthForIndex, oldRect.size.height);
                }else{
                    fullRect = CGRectMake(xOffset, 0, widthForIndex + self.DynamicSpace, oldRect.size.height);
                }
            ...
            if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamic) {
                ...
                for (NSNumber *width in self.segmentWidthsArray) {
                    ...
                    if (i+1 == self.sectionTitles.count) {
                        ...
                        selectedSegmentOffset = selectedSegmentOffset + [width floatValue] + self.DynamicSpace;
                    }
                    i++;
                }
                return CGRectMake(selectedSegmentOffset + self.selectionIndicatorEdgeInsets.left + self.DynamicHeaderTailer
                                  ,indicatorYOffset
                                  ,[[self.segmentWidthsArray objectAtIndex:self.selectedSegmentIndex] floatValue] - self.selectionIndicatorEdgeInsets.right
...
- (void)updateSegmentsRects {
    ...
    if (self.type == HMSegmentedControlTypeText && self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
        ...
        CGFloat titleWidth = [[self.segmentWidthsArray valueForKeyPath:@"@sum.self"] floatValue] + (self.sectionTitles.count-1)*self.DynamicSpace;
        
        if (_alignment == HMSegmentedControlSegmentAlignmentCenter) {
            if (titleWidth < self.frame.size.width) {
                self.DynamicHeaderTailer = (self.frame.size.width - titleWidth)/2.0f;
            }
        }else if (_alignment == HMSegmentedControlSegmentAlignmentLeft){
            self.DynamicHeaderTailer = OldDynamicHeaderTailer;
        }
...
- (void)setDynamicHeaderTailer:(CGFloat)DynamicHeaderTailer{
    OldDynamicHeaderTailer = DynamicHeaderTailer;
    _DynamicHeaderTailer = DynamicHeaderTailer;
}
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(enlargeRect, touchLocation)) {
        ...
        if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
            ...
            for (NSInteger i = 0; i < self.segmentWidthsArray.count; i++) {
                
                NSNumber *width = [self.segmentWidthsArray objectAtIndex:i];
                
                if (i == 0) {
                    widthLeft = widthLeft - [width floatValue] - self.DynamicHeaderTailer - self.DynamicSpace/2.0;
                }else if (i + 1 == self.segmentWidthsArray.count) {
                    widthLeft = widthLeft - [width floatValue] - self.DynamicSpace/2.0;
                }else{
                    widthLeft = widthLeft - [width floatValue] - self.DynamicSpace;
                }
...
- (CGFloat)totalSegmentedControlWidth {
    if (self.type == HMSegmentedControlTypeText && self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
        ...
        return [[self.segmentWidthsArray valueForKeyPath:@"@sum.self"] floatValue] + (self.sectionTitles.count-1)*self.DynamicSpace + 2*self.DynamicHeaderTailer;
    } else {
...
- (void)scrollToSelectedSegmentIndex:(BOOL)animated {
    ...
    if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
        ...
        for (NSNumber *width in self.segmentWidthsArray) {
            ...
            if (i+1 == self.sectionTitles.count) {
                ...
                offsetter = offsetter + [width floatValue] + self.DynamicSpace;
            }
            i++;
        }
    ...
    rectToScrollTo.origin.x = rectToScrollTo.origin.x - selectedSegmentOffset;
